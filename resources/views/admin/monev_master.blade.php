                ...
                <div class="card-header">
                    ...
                    <div class="col-md-8" style="margin-top: 10px; margin-bottom: 10px;">
                        <button type="button" class="btn btn-primary buttonedit" data-toggle="modal" data-target="#myModal"
                            data-prodi="" data-id="" data-url="{{ route('monev.master.store') }}">
                            Tambah Data {{ $title }}
                        </button>
                    </div>
                    <!-- Modal -->
                        ...
                        <tbody>
                            ...
                            @foreach ($monev as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->nama }}</td>
                                    <td>
                                        <form action="{{ route('monev.master.destroy', $item->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <a href="" class="btn btn-sm btn-outline-success buttonedit"
                                                data-toggle="modal" data-target="#myModal" data-monev="{{ $item->nama }}"
                                                data-id="{{ $item->id }}"
                                                data-url="{{ route('monev.master.update', $item->id) }}">
                                                Edit
                                            </a> |
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.buttonedit', function() {
            ...
            var data_monev = $(this).attr('data-monev');
            var data_url = $(this).attr('data-url');
            $('.monev').attr('value', data_monev);
